...
setup:
	...
	docker exec -it postgres_dst psql -d airflow -U airflow -c "CREATE TABLE IF NOT EXISTS sales(id SERIAL,quantity INT,price INT,date DATE,PRIMARY KEY (id));"

inspect:
	docker exec -it postgres_dst psql -d airflow -U airflow -c "SELECT * FROM sales;"
